...
| [x] | CKB-04 | Reconcile and deduplicate social candidates against the 157 merged NLB/ActiveSG drafts; select a hobby-diverse 40–50-row real seed across the three target areas. | Reconciled | The queue contains 355 unique candidates: 157 merged drafts and 198 public leads after 34 duplicate rows/reposts were removed. The deterministic 45-row shortlist is split 25/10/10 across Jurong West/Punggol/Bishan and 13 social, 10 NLB, 8 ActiveSG-draft and 14 official-web candidates, spanning all four hobby buckets. Expired, closed, adult-only, non-hobby, unavailable and out-of-area rows fail selection. |
| [ ] | CKB-05 | Provide a human verification queue/attestation flow and promote only rows whose source, date, price, age, venue, participation, and schedule were actually checked. | Waiting on human review | `scripts/promote_ckb_shortlist.py` reuses the canonical row validator and fails closed on pending rows, undocumented rejections, automated reviewer identities and incomplete approvals. All 45 rows are intentionally pending; no automated actor has been recorded as a verifier. |
| [ ] | CKB-06 | Build and commit `data/seed_ckb.json` with the merged builder; reconcile quarantine, freshness, URL, provenance, provider, vibe, regional, and long-tail coverage. | Planned | Default build succeeds without `--allow-incomplete`; all coverage checks pass. |
...
| [ ] | CKB-08 | Add a clean-runtime real-row planning smoke test that imports no synthetic evaluation catalogue. | Harness reconciled; canonical run pending | A clean subprocess plans sourced rows to the expected trusted-adult checkpoint and asserts `sim.catalogue` is never imported. The same smoke must pass against the human-attested canonical artifact before closure. |
| [ ] | CKB-09 | Run CKB URL checks, focused tests, full tests, clean-runtime/API smoke, and a final provenance/privacy/copyright review. | Partially reconciled; canonical URL pass pending | All 121 tests pass; changed Python files pass Ruff; the clean runtime smoke passes; secret/diff checks are clean. Collection is public-only and excerpt-capped. URL checks against the final promoted seed remain dependent on human attestation. |
| [ ] | CKB-10 | Reconcile `README.md`, `seed-ckb.md`, `outstanding.md`, and this checklist; then commit, push, and open a PR to `main`. | Docs reconciled; publication pending | README, seed runbook, OW-17 and this checklist describe the queue, sign-off boundary and readiness semantics. The branch can be pushed as a checkpoint, but OW-17 and its final PR remain open until the attested artifact passes CKB-06/08/09. |
