...
| **OW-05** | P0 | **In progress** | Build the Hobbi PoC | Codex | Before demo recording | The canonical environment installs; Intake/Setup, I0, six agents, G1–G4, both stores, two request loops and the longitudinal cycle run through the documented happy and failure paths | Implemented, reviewed and reconciled in [PR #4](https://github.com/respoopu/agentic_ai_hackathon_2026/pull/4); 83 tests and the local API smoke path pass. Remains open until merge. |
| **OW-06** | P0 | **In progress** | Implement tests, simulation and one-command report | Codex | Before slide 7 freezes | Family A invariants pass; eligible and boundary profiles stay separate; counterfactual and longitudinal runs execute; `python -m sim.report` emits every reported slide metric with denominators | Runtime A1–A12 tests, 12 eligible profiles, separate age matrix, 12-cycle replay, counterfactual and B1–B15 report are implemented. B6 remains explicitly unmeasured pending real judge/human scoring. |
| **OW-15** | P0 | **In progress** | Resolve Broker authorization and idempotency contract | Codex | Before Broker implementation in OW-05 | `BookingRecord` carries `guardian_verdict_id`; G3 requires an approved verdict matching `plan_id`; Broker derives or durably reserves one stable transaction id per logical commitment; G4 permits replay while applying the commitment exactly once; architecture, protocol, prompts and fixtures agree | Code and runtime tests enforce matching authorization, stable per-item IDs, atomic multi-item commit and sequential/concurrent replay. Merged prompt files remain unchanged as directed, so source-document closure remains open. |
| **OW-16** | P0 | **In progress** | Enforce fixture store and gate boundaries | Codex | Before OW-06 results are trusted | The validator checks every fixture's declared `store_reads`, `store_writes` and `gates` against the agent/scenario contract, includes Intake/Setup in the permission matrix, and mutation tests prove forbidden reads/writes and skipped gates fail | Implemented in [PR #4](https://github.com/respoopu/agentic_ai_hackathon_2026/pull/4) with Intake in the runtime matrix plus forbidden-read/write and required-gate mutation tests. Remains open until merge. |
