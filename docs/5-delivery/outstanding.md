...
| **OW-15** | P0 | **Not started** | Resolve Broker authorization and idempotency contract | — | Before Broker implementation in OW-05 | `BookingRecord` carries `guardian_verdict_id`; G3 requires an approved verdict matching `plan_id`; Broker derives or durably reserves one stable transaction id per logical commitment; G4 permits replay while applying the commitment exactly once; architecture, protocol, prompts and fixtures agree | [`discrepancies.md`](../4-decisions/discrepancies.md) E2 · [`architecture.md`](../3-system/architecture.md) §§3.7, 5 · [`evaluation.md`](../3-system/evaluation.md) A7 |
| **OW-16** | P0 | **Not started** | Enforce fixture store and gate boundaries | — | Before OW-06 results are trusted | The validator checks every fixture's declared `store_reads`, `store_writes` and `gates` against the agent/scenario contract, includes Intake/Setup in the permission matrix, and mutation tests prove forbidden reads/writes and skipped gates fail | [`fixture-schema.md`](../../tests/agent-system-prompts/fixture-schema.md) · [`shared-protocol.md`](../agent-system-prompts/shared-protocol.md) §System boundary and permissions |
