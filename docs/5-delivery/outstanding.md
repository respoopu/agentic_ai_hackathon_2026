...
| **OW-06** | P0 | **In progress** | Implement tests, simulation and one-command report | Codex | Before slide 7 freezes | Family A invariants pass; eligible and boundary profiles stay separate; counterfactual and longitudinal runs execute; `python -m sim.report` emits every reported slide metric with denominators | Implementation and Claude-review reconciliation are complete in PR #7, pending merge; 149 tests pass. B10 reports 0/8 adversarial plus 0/12 eligible diagnostics, B11 is causally resolved 1/1, B14 is 8/12 vs 2/12, and B15 is 4/4 vs 3/4 using planned session dates. B12 is only branch reachability; OW-23 tracks product measurement. B1/B4 remain tied to OW-22, B6 to deck scoring and B13 to a dated link sample. |
