...
| **OW-17** | P0 | **In progress** | Publish the canonical CKB runtime artifact | Codex + named human reviewer | Before demo recording | The merged builder emits `data/seed_ckb.json`; a clean runtime loads sourced non-fictional rows; health reports `ready_for_real_planning=true`; and a real-row planning smoke test passes without synthetic catalogue data | Branch `codex/ow-17-canonical-ckb-demo` now produces a 355-candidate queue and a balanced 45-row shortlist from merged drafts, public community previews and official web sources. Duplicate, expired, non-hobby, adult-only, unavailable and out-of-area candidates are filtered. Promotion and health/smoke guardrails are implemented; a named human must now check the 45 source links and confirmed fields before the canonical artifact can be published. |
