...
| **OW-04** | P0 | **In progress** | Reconcile and integrate `feat/seed-ckb` through PR #3 | Rayden | Before PoC implementation | PR #3 is aligned with merged v2.2; stored and hydrated listing schemas agree; deterministic build/load boundaries and fixtures pass tests; stale branch wording and PR claims are corrected; PR #3 is merged | Claude's follow-up review was remediated and re-audited on 1 Sep; merge remains. [`architecture.md`](../3-system/architecture.md) §§5, 9.3, 11 |
