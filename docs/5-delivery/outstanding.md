...
| **OW-06** | P0 | **In progress** | Implement tests, simulation and one-command report | Codex | Before slide 7 freezes | Family A invariants pass; eligible and boundary profiles stay separate; counterfactual and longitudinal runs execute; `python -m sim.report` emits every reported slide metric with denominators | Implementation and reconciliation are complete on `codex/ow-06-executable-evaluation`, pending merge. The report now executes all eight adversarial scenarios, a 12-cycle real-agent Observer replay and the S$0 static counterfactual; 144 tests pass. B1/B4 remain tied to OW-22, B6 human scoring belongs to deck work, and B13 requires a dated link sample rather than synthetic data. |
