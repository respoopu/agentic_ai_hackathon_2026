...
4. A Guardian-passed Plan crosses G3 to sandboxed Broker. Broker binds the booking to that verdict and commits the ledger once using `ledger_version` plus a stable transaction id derived from each logical commitment. It emits `BookingRecord` and both teen/parent artefacts, then crosses G4 to Observer; exact replay returns the stored record without another effect.
